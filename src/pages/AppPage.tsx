    ...
    <aside className="bg-card border border-border rounded-2xl p-5 h-fit lg:sticky lg:top-24">
      ...
      {(v.rol_psm_dsm !== 'none' || s.psmHorasMes > 0 || s.dsmHorasMes > 0) && (
        <>
          ...
          {s.dsmHorasMes > 0 && v.rol_psm_dsm !== 'dsm' && <Row label="Horas DSM puntuales" sub={`${formatHours(s.dsmHorasMes)} × ${formatEur(v.plus_dsm / 155.63)}/h`} value={formatEur(s.importeDsmDia)} />}
        </>
      )}
